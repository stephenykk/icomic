...
async function handleChunk(chunk) {
    try {
        await Promise.allSettled(chunk.map(cartoon => await spawnCommand('bash', ['./comic/autoCartoonNeed.sh', cartoon.name, cartoon.needs.join(',')])))
    }catch(err) {
...
async function main() {
    ...
    if (needCartoons.length === 0) {
        console.log('==============> there is no any need to down :)', needDownData);
        return
    }
